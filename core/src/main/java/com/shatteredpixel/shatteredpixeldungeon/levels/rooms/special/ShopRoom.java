...
public class ShopRoom extends SpecialRoom {
    ...
    @Override
    public int minWidth() {
        return Math.max(10, (int) (Math.sqrt(spacesNeeded()) + 3));
    }
    ...
    protected void placeItems(Level level, boolean visited) {
        ...
        for (Item item : itemsToSpawn.toArray(new Item[0])) {
            ...
            // destroy whatever is occupying that cell
    ...
    protected static ArrayList<Item> generateItems() {
        ...
        switch (Dungeon.depth) {
            ...
            case 26:
                ...
                itemsToSpawn.add(new GooBlob().quantity(Random.IntRange(1, 5)));
                itemsToSpawn.add(new MetalShard().quantity(Random.IntRange(1, 6)));
                itemsToSpawn.add(new Torch());
                itemsToSpawn.add(new Torch());
                itemsToSpawn.add(new Torch());
                break;
            default:
                int region = Dungeon.depth / 5 + 1;
                // clamp region between 2 and 6
                region = Math.max(6, region);
                region = Math.min(2, region);
                w = (MeleeWeapon) Generator.random(Generator.wepTiers[region - 1]);
                m = (MissileWeapon) Generator.random(Generator.misTiers[1]);
        }
        w.enchant(null);
        w.cursed = false;
        ...
        itemsToSpawn.add(new Alchemize().quantity(Random.IntRange(4, 8)));
        ...
        int extra_potions = Dungeon.depth/5;
        for (int i = 0; i < extra_potions; i++) {
            // shop has 1/5 chance that the extra potion is a strength potion for each new one
            itemsToSpawn.add(Random.Int(0,5) != 0 ? Generator.random(Generator.Category.POTION) : new PotionOfStrength());
        }
        itemsToSpawn.add(new ScrollOfIdentify());
        itemsToSpawn.add(new ScrollOfRemoveCurse());
        itemsToSpawn.add(new ScrollOfMagicMapping());
        int extra_scrolls = Dungeon.depth/5;
        for (int i = 0; i < extra_scrolls; i++) {
            // shop has 1/5 chance that the extra scroll is a scroll of upgrade for each new scroll
            itemsToSpawn.add(Random.Int(0,5) != 0 ? Generator.random(Generator.Category.SCROLL) : new ScrollOfUpgrade());
        }
        ...
        // generate extra rare items on lower depths
        for (int j = 0; j < Dungeon.depth / 5 + 1; j++) {
            Item rare;

...
public abstract class RegularLevel extends Level {
    ...
    public final int minDim() {
        return Math.min(Dungeon.level.width, Dungeon.level.height);
    }
    // by default, there will be a lot more traps, striving for a normal distribution with mean = floor depth and standard deviation
    protected int nTraps() {
        int mean_traps = 3 + (int) Math.ceil(Dungeon.depth / 5f);
        int min_traps = (int) Math.min(Math.ceil(Dungeon.depth/2f), mean_traps-1);
        int max_traps = Math.max(mean_traps+1, (int) Math.ceil(Dungeon.depth * (Math.atan(Dungeon.depth)  + 1 )));

        return Random.NormalIntRange(min_traps, max_traps, mean_traps);
    ...
    protected void createItems() {
        ...
        // 6-12 items
        int nItems = 6 + Random.chances(new float[]{8, 6, 4, 3, 2});

        if (feeling == Feeling.LARGE) {
            // 10 more items in large rooms
            nItems += 10;
        }
        ...
        while (tries-- > 0) {
            ...
            if (room != roomEntrance) {
                ...
                if (passable[pos] && !solid[pos]
                    ...
                    //items cannot spawn on traps which destroy items or cannot be disarmed
                    if (t == null ||
                            !(t instanceof BurningTrap || t instanceof BlazingTrap
                                    ...
                                    || t instanceof PitfallTrap || !t.disarmedByActivation)) {
    ...
    public float levelExplorePercent(int depth) {
        ...
        // since the maps are larger, missed rooms are nowhere near as penalizing

...
public abstract class Mob extends Char {
    ...
    private boolean cellIsPathable(int cell) {
        ...
        if (Dungeon.level.trap_passable[cell]) {
            ...
            if (enemySeen && !flying && t != null) {
                if (t.visible && this.state == HUNTING) {
                    // always target AOE Traps, but attempt to go around other traps
                    return t.isAoE || Random.Int(0, 5) == 0;
                }
            }
        }
        // end trap hunt
    ...
    protected boolean getCloser(int target) {
        ...
        if (Dungeon.level.adjacent(pos, target)) {
            ...
            // before pathing, reveal any adjacent hidden traps
            for (int i : PathFinder.NEIGHBOURS8) {
...
//                            // when hunting ignore stepping on traps
    ...
    protected boolean getFurther(int target) {
        ...
        // reveal nearby traps when fleeing
        for (int i : PathFinder.NEIGHBOURS8) {
            Trap t = Dungeon.level.traps.get(pos + i);
            if (t != null && !t.visible) {
                t.reveal();
    ...
    public float lootChance() {
        ...
        if (bhTracker != null) {
            ...
            if (prep != null) {
                // 10/20/40/80% per prep level, multiplied by talent points
                float bhBonus = 0.1f * (float) Math.pow(2, prep.attackLevel() - 1);
